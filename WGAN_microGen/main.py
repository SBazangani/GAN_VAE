...
for epoch in range(num_epochs):
    ...
    for batch_idx, image_batch in enumerate(data_loader):
        ...
        #dis_real_loss_JSD = criteriond1_JSD(dis_real_out, label_one_smoothed)
        torch.cuda.empty_cache()

        # 1.2 Train discriminator on fake data from generator

        noise = torch.normal(0, 0.1, (inputs.shape), device=device).detach()
        dis_inp_fake_x = gen(noise,h)
        dis_fake_out = discrim(dis_inp_fake_x,h)
        ...
        gen.zero_grad()
        gen_out = gen(noise, h)
        dis_out_gen_training = discrim(gen_out, h)
        ...
        #gen_loss =  gen_loss +  gen_loss_JSD
